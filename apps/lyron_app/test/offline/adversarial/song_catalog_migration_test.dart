...
void main() {
  ...
  test('an existing pre-migration (v3) catalog database gains '
      'pendingEmptyConfirmationAt on upgrade and keeps its populated rows '
      'intact (D4, local-data-durability-contract Task 3.1)', () async {
    ...
    final mutationRows = await db.select(db.cachedCatalogSongMutations).get();
  ...
  test('an existing pre-migration (v4) catalog database gains sourcesEvictedAt '
      'on upgrade and keeps its populated rows intact (D6, '
      'local-data-durability-contract Task 3.4)', () async {
    ...
    expect(
      afterUpdate.sourcesEvictedAt!.isAtSameMomentAs(DateTime.utc(2026, 8, 22)),
      isTrue,
    );
