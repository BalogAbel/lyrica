...
void main() {
  group('PlanningMutationSyncController', () {
    test('successful sync refreshes projection and clears the mutation', () async {
    ...
    test('authorization failures move mutations out of normal overlay states', () async {
      final store = _FakePlanningMutationStore(
        pending: [
          PlanningMutationRecord(
            aggregateId: 'plan-1',
      ...
      expect(store.lastSavedStatus, PlanningMutationSyncStatus.failedAuthorization);
    });
    ...
    test('retrying a failed mutation marks it pending and resubmits it', () async {
      final store = _FakePlanningMutationStore(
        pending: [],
        all: [
          PlanningMutationRecord(
...
class _FakePlanningMutationStore implements PlanningMutationStore {
  ...
  @override
  Future<bool> hasUnsyncedMutations({required String userId}) async => pending.isNotEmpty;

  @override
  Future<List<PlanningMutationRecord>> readAllMutations({required String userId, required String organizationId}) async => all;

  @override
  Future<PlanningMutationRecord?> readMutation({required String userId, required String organizationId, required String aggregateId}) async {
  ...
  @override
  Future<List<PlanningMutationRecord>> readPendingMutations({required String userId, required String organizationId}) async => pending;
