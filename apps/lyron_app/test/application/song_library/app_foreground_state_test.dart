...
void main() {
  ...
  group('WidgetsBindingAppForegroundState', () {
    test(
      'reports foreground when constructed before the binding has settled, '
    ...
    test(
      'a genuine backgrounding transition after an optimistic start still '
      'flips isForeground to false and emits on the stream',
      () async {
        TestWidgetsFlutterBinding.instance.handleAppLifecycleStateChanged(
          AppLifecycleState.hidden,
        );
    ...
    test(
      'the first real lifecycle callback reporting resumed after an '
      'optimistic start is a no-op: no spurious stream emission',
      () async {
        TestWidgetsFlutterBinding.instance.handleAppLifecycleStateChanged(
          AppLifecycleState.hidden,
        );
