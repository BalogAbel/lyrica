...
void main() {
  test('browse rows join mutations and filter by current browse state', () async {
    ...
    expect(
      container.read(songLibraryBrowseRowsProvider).map((row) => row.song.title),
      ['Alpha', 'Beta', 'Gamma'],
    );

    container.read(songLibraryBrowseControllerProvider.notifier).setQuery('mm');
    expect(
      container
  ...
  test('browse rows ignore stale mutations from another organization', () async {
    final container = ProviderContainer(
      overrides: [
        songLibraryListProvider.overrideWith(
          (ref) async => const [
