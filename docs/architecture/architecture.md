...
- `presentation`: routes, screens, controllers, UX state. Each area keeps its
  ...
  Provider failures are terminal: every async provider opts out of Riverpod 3's
  default automatic retry, so a failure settles into `AsyncError` immediately
  and reaches its failure surface instead of hiding behind a retrying spinner.
  Retrying is a user action
  ([ADR-032](decisions/ADR-032-no-automatic-provider-retry.md))
...
Song sync and discard have a context-wide ownership boundary keyed by the complete `(userId, organizationId)` pair because a sync snapshots every pending song in that context. Sync and discard are mutually exclusive inside one context, while different contexts remain independent. Concurrent sync requests share the owned run; sync requests arriving while discard owns the context share one queued run, wait for lease release, and only then snapshot pending work. This boundary covers sync and discard only: "keep mine" conflict resolution (an explicit remote overwrite) is not admitted into the same context lease and can run concurrently with an owned sync or discard on the same context. That is a pre-existing gap, not something the ownership boundary claims to close (ADR-013).
...
Local storage growth is bounded by two policies, not by keeping "one active snapshot": a mutation budget (LF-T3) and a storage-pressure recovery policy (LF-T4), specified together because catalog eviction cannot relieve the mutation budget — pending mutations are never evictable, so the only remedy for a full mutation store is to sync or discard. `BudgetedPlanningMutationStore` measures the planning mutation store's content-derived byte footprint before every write that can grow it and refuses new writes past a hard threshold. Storage pressure works differently: a measured critical total only raises the pressure classification the sync overview displays; it never evicts by itself. The one production eviction trigger is a write that actually fails at the storage layer — that failure evicts droppable catalog data once and retries the write once, regardless of what the last measured pressure was. The protection order never evicts pending planning or song mutations, the planning projection (offline it is the only readable view), cached catalog summaries (they back the browsable list), or the last-known identity store (ADR-020); the only thing eviction ever drops is cached catalog **sources** for songs with no pending song mutation, since a song's body is the largest cached payload and is re-fetchable on reconnect. Accounting is derived from row content (SQL `length(...)` over text columns) rather than a platform file-size or quota API, so the same logic runs on native and web; every threshold and the eviction trigger are verified against the native Drift/sqlite3 backend only, and the web/IndexedDB assumptions behind them remain unverified pending `docs/deferred/2026-06-29-web-offline-e2e.md`. Every committed change to the measured footprint — planning mutation writes, projection updates, catalog snapshot/mutation changes, and an eviction that actually removed rows — advances a monotonic revision that the sync overview's storage figure watches, so a mounted listener remeasures after real commits instead of showing a figure from whenever it happened to be built; a delete/clear that affected no rows and an idempotent upsert with an unchanged payload advance nothing. The evict-once/retry-once recovery on a failed storage write is one shared boundary (`LocalStorageWriteRecovery.guard`) used by every local write that can grow stored bytes — planning mutation writes, the planning projection, and catalog snapshot/mutation/reconcile writes alike, not only the mutation-budget path — and a caller-declared domain rejection (a slug conflict, or a superseded projection refresh) passes through it untouched instead of being misread as storage pressure. The mutation budget's own measure-check-write sequence is serialised per `(userId, organizationId)` so concurrent writes cannot overshoot the documented one-mutation bound, and a delete that collapses a still-pending create is admitted regardless of the budget because it shrinks the pending set rather than growing it. See ADR-028.
...
`PlanningMutationSyncController._run` sends a snapshot of a pending mutation and then awaits the backend; the remote wait is deliberately outside the per-`(userId, organizationId)` write queue described above, because holding a local write lock across a network call would block every local edit for the duration of a sync. The window this opens — a local edit landing on the same aggregate while the remote call is in flight — is closed by a `localRevision` integer that `CachedPlanningMutations` bumps on every local write to a row, local bookkeeping only and unrelated to `baseVersion`/`version` (which track the server's view). The sync captures a row's `localRevision` when it snapshots and passes it back into `saveSyncAttemptResult(accepted)` and the `clearMutation` that follows a successful refresh or reconcile; each write applies only if the row's revision still matches, as part of the same storage statement — not a preceding read-then-decide in Dart, which would reopen the same window. A stale conclusion is not an error: the accepted remote write is not undone (it happened), the row is simply left pending with the newer local content, already exactly where the next sync needs it. The song/catalog sync path closes the identical window the same way: `CachedCatalogSongMutations` carries its own `localRevision`, and `reconcileSyncedSong` — which deletes the mutation row and reconciles the cached snapshot as one combined write, since songs have no separate accept-then-clear step — applies only if the revision still matches, skipping the whole reconcile (not only the delete) when it does not. A local edit made anywhere during a sync's remote round trip is therefore never lost, in either domain: it survives as a pending mutation and rides the next sync. See ADR-030, including its fold-status follow-up, which closed a related gap where a local edit folding onto a planning mutation row already marked accepted-but-not-yet-cleared kept that stale status instead of reverting to pending.

A delete issued while the create that introduced the row is still in flight is likewise never lost. The `localRevision` gate above assumes newer intent leaves a higher-revision *pending* row, which holds for an edit but not for a delete of a still-pending create, since ADR-028 D10's collapse removes the row instead of bumping it. The sync now writes a durable `sending` marker immediately before a create's remote call — the mirror of the `accepted` marker it already writes after — so a delete arriving mid-flight can tell the create is genuinely in progress and keep the row as a `cancelling` tombstone instead of physically collapsing it; the tombstone is invisible to every merged read from the moment it is written. Once the create's own remote call concludes, the tombstone resolves: a succeeded create converts it to a real pending delete that the next sync sends, and a failed create discards it outright, exactly as a plain collapse would have. A record left `sending` by a crash is treated as pending and resent, not stranded. Planning has one further window with the identical shape but no live call to race — a create already `accepted` but not yet locally cleared — which converts straight to a real pending delete with no tombstone needed, using the create's own pre-accept `baseVersion` and letting the backend's version guard turn a stale value into a visible conflict rather than a silent wrong delete; this window does not exist on the song side, whose `reconcileSyncedSong` has no separate accept-then-clear step to be caught between. Separately, a mutation row that vanishes while a sync attempt for it is concluding (an ordinary consequence of the collapse above, not a defect) is reported as "did not apply" rather than thrown as an error, so it no longer aborts the rest of that sync pass. See ADR-030's in-flight create cancellation follow-up.
Local data access is decoupled from live auth-session validity (ADR-020). `AppAuthStatus.sessionExpired` is a first-class offline-authenticated / re-auth-required state: a previously authenticated user keeps reading cached songs and plans and keeps queueing writes while the session is not live, both for in-session expiry and for offline cold-start relaunch with a dead token. Cold start resolves which user to load from a durable `LastKnownIdentity { userId, email, organizationId, updatedAt }` Drift record written on every successful sign-in; a `null` restored session with a known identity maps to `sessionExpired` (carrying `lastKnownSession`), while `null` with no identity maps to `signedOut`. The router keeps offline-authenticated users in the app (membership resolves via the cached organization id) and surfaces a persistent re-auth banner. The offline-authenticated read path itself is local-database-driven, not session-driven: in `sessionExpired`, `SongCatalogController.handleOfflineAuthenticated()` and `PlanningSyncController.handleOfflineAuthenticated()` each independently resolve their read context from `LastKnownIdentity`, requiring only a matching local snapshot (catalog) or projection (planning) for that `(userId, organizationId)` pair — no network call, no live session check. This is deliberately redundant with the `sessionExpired`-vs-`signedOut` mapping above: even if a purge path were ever reintroduced upstream, the read path itself no longer depends on anything that can expire. See ADR-035 for the full local-data purge contract this belongs to (the enumerated purge reasons and decisions D1–D8); as of this writing only the read-path/auth-state half (D1–D3) is built, and the structural `LocalDataLifecycle` gate (D7) unifying every purge call site behind one audited seam is a later phase, not yet implemented. Outside the explicitly confirmed different-user path described below, local data and the identity record are wiped only on explicit sign-out and authoritative verified-empty membership revocation, never on connectivity-driven or unknown session loss. While offline, cached data may outlive a server-side revocation by design; authorization stays backend-enforced and converges on reconnect when `verifiedEmptyMembership` fires. The different-user re-auth resolution runs on the live `signedIn` edge (ADR-029): a same-user re-sign-in flushes; a different user with no user-wide pending local work is wiped and proceeds without a prompt; a different user with user-wide pending or unreadable-count work is confirmed first; and cancelling signs the new session out and returns the app to being offline-authenticated as the prior user with nothing deleted. The count and cleanup cover every locally retained organization for the prior user, without claiming an atomic transaction across the song and planning databases. Every signed-in notification synchronously claims a new generation and supersedes any pending prompt before queueing its captured session. Queued work remains current only while that generation and the live signed-in `userId`/`email` still match; currentness is rechecked after waits and immediately before deletion, backend sign-out, identity clear, and identity writes. A failed last-moment check produces the typed `ReauthSuperseded` outcome and no side effect rather than a stale wipe or cancellation. The root `ConsumerStatefulWidget` hosts the prompt through a single Riverpod `listenManual(..., fireImmediately: true)` subscription, captures the current value immediately, and presents it post-frame exactly once per stable request id; answers carry that id so an obsolete dialog cannot answer a newer request. A confirmed different-user wipe is a fourth destructive trigger alongside explicit sign-out and authoritative revocation — it is not a relaxation of the non-destructive-on-uncertainty rule above, because it only fires after an explicit confirmation, never from an unknown or connectivity-failed session.
