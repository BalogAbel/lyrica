...
No `accepted` state exists today: `PlanningMutationSyncStatus` is `{pending, failedAuthorization, failedDependency, failedRemoteDelete, conflict}` (`planning_mutation_sync_types.dart:15-21`). This step adds a durable `accepted` marker. **No Drift migration is required**: `syncStatus` is persisted as a free-text column (`drift_planning_mutation_store.dart:423`, `.equals(PlanningMutationSyncStatus.pending.value)`), so a new enum value plus its `value` / `fromValue` string mapping is sufficient — the existing column stores it without a schema change. The marker is written via `saveSyncAttemptResult(syncStatus: accepted)` the moment `syncMutation` returns, before `clearMutation`. On a subsequent run, a mutation already marked `accepted` is reconciled/cleared rather than re-sent, so a crash between accept and clear no longer produces a false conflict. The merge (Step 5) treats `accepted` as still-applied-locally so the edit does not flicker.
...
- Unit (`LF-1`): simulated crash between accept and clear → next run does not re-send and does not surface a false conflict; the `accepted` marker round-trips through the store (`value`/`fromValue` mapping, persisted in the existing text column — no migration).
