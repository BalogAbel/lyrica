...
- Observability foundation (ADR-036): zone-based span propagation, the PII scrub policy, `traceparent` injection, `initObservability`/`runBootstrapGuarded`, and the instrumented catalog refresh, as listed in `docs/specs/2026-08-28-observability-foundation.md` ("Testing strategy"). **Tests that initialise the Sentry SDK must be offline:** the SDK swaps `NoOpTransport` for a real `HttpTransport` as soon as a DSN is set, so such tests install a recording `options.transport` and a global `HttpOverrides` guard that fails on any HTTP attempt. **Tests that wait for a Sentry transaction/event to be delivered must wait on a condition** (poll with real timers under a deadline, like `pumpUntil` in `sentry_observability_test.dart`, or a `Completer` fired from `beforeSendTransaction`), never a fixed number of event-loop hops: the latter flaked in CI (`report_uncaught_zone_error_sentry_test.dart`, `sentry_observability_test.dart` and the catalog instrumentation test all follow this; a negative assertion such as `isEmpty` cannot wait on a condition and keeps a real delay of at least 100 ms instead). The PII scrub also has a committed seeded property fuzzer (`sentry_pii_scrub_fuzz_test.dart`: a planted secret (URL userinfo/query/fragment, JWT, `sb_secret_`, credential key/value text in snake_case, camelCase, `_`-prefixed, quoted and spaced forms) never survives, benign text is unchanged, idempotent, never throws; opt-in `--dart-define=SCRUB_FUZZ_CASES=2000000` for a local sweep).
