...
- slug reconciliation after backend returns canonical slug, including the fallback path where the accepted write must reconcile locally before a later full refresh succeeds while the same active planning boundary still owns the projection
...
- auth-boundary cleanup for explicit sign-out, session expiry or revoked auth session, authenticated account switch, and active-organization switch, including cancellation of stale cleanup work after a newer signed-in generation takes ownership
